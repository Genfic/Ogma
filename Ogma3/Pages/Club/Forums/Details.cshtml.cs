...
public sealed class DetailsModel(ClubRepository clubRepo, AppDbContext context) : PageModel
{
	...
	public async Task<IActionResult> OnGetAsync(long threadId, long clubId)
	{
		var clubThread = await context.ClubThreads
			...
			.Select(ct => new ThreadDetails
			{
				...
				{
					Id = ct.CommentThread.Id,
					Type = CommentSource.ForumPost,
					LockDate = ct.CommentThread.LockDate,
				},
			})
			.FirstOrDefaultAsync();
