...
public partial class ChapterEntityType
{
    public static RuntimeEntityType Create(RuntimeModel model, RuntimeEntityType baseEntityType = null)
    {
        ...
        var signature = runtimeEntityType.AddProperty(
            "Signature",
            typeof(uint[]),
            propertyInfo: typeof(Chapter).GetProperty("Signature", BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly),
            fieldInfo: typeof(Chapter).GetField("<Signature>k__BackingField", BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly));
        signature.SetValueConverter(new ValueConverter<uint[], int[]>(int[] (uint[] v) => Array.ConvertAll(v, int (uint i) => ((int)i)), uint[] (int[] v) => Array.ConvertAll(v, uint (int i) => ((uint)i))));
        signature.SetComparer(new ValueComparer<uint[]>(
            bool (uint[] a, uint[] b) => a == null && b == null || a != null && b != null && a.SequenceEqual(b),
            int (uint[] a) => a.Aggregate(0, int (int i, uint v) => HashCode.Combine(i, v.GetHashCode())),
            uint[] (uint[] v) => v));
        signature.AddAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.None);
        signature.AddAnnotation("Relational:ColumnType", "integer[]");
